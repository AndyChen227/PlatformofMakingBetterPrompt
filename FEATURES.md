...
**三步正则处理**（`PunctuationNormalizerRule.java`，按实际执行顺序）：

1. **压缩重复感叹号**：正则 `!{2,}` → `!`
   - 触发条件：存在 2 个或以上连续感叹号
   - Change 消息：`[punctuationNormalizer] 压缩重复感叹号: '!!' → '!'`

2. **压缩重复问号**：正则 `\?{2,}` → `?`
   - 触发条件：存在 2 个或以上连续问号
   - Change 消息：`[punctuationNormalizer] 压缩重复问号: '??' → '?'`

3. **规范省略号**：正则 `\.{4,}` → `...`
   - 触发条件：存在 4 个或以上连续英文句点
   - Change 消息：`[punctuationNormalizer] 规范省略号: '....' → '...'`

**跳过条件**：若三步均无变化，Changes 列表记录 `[punctuationNormalizer] 未检测到需要规范化的标点`
...
- 期望输出：`"Is this correct? I'm sure! Let me think..."`
- 期望改动：`["[punctuationNormalizer] 压缩重复问号: '??' → '?'", "[punctuationNormalizer] 压缩重复感叹号: '!!' → '!'", "[punctuationNormalizer] 规范省略号: '....' → '...'"]`
...
**完整位权解析器**（`NumberNormalizerRule.java`）：

支持词汇范围：
- ones：zero, one, two, … nineteen（共 20 个）
...
1. **百分比优先**：先用 `PERCENT_PATTERN`（数字词组 + `percent` / `percentage`）替换
   - 示例：`fifty percent` → `50%`

2. **普通数字**：再用 `NUMBER_PHRASE_PATTERN`（纯数字词组）替换
   - 示例：`two hundred and fifty three` → `253`
   - 示例：`one million two hundred thousand` → `1200000`

**解析算法**（`parseNumberPhrase()`）：
- 维护两个累加器：`current`（当前段）、`result`（已完成段）
- 遇到 `hundred`：`current = current * 100`（current 为 0 时视为 1）
- 遇到 `thousand`：`result += current * 1000`，重置 `current = 0`
- 遇到 `million`：`result += current * 1000000`，重置 `current = 0`
- 最终返回 `result + current`

**Changes 消息**：`[numberNormalizer] 'matched phrase' → 'numeric'`；若无命中：`[numberNormalizer] 未检测到可转换的英文数字`
...
| 输入 | 期望输出 |
|------|---------|
| `"I need two hundred tokens"` | `"I need 200 tokens"` |
| `"Use fifty percent of the budget"` | `"Use 50% of the budget"` |
| `"two hundred and fifty three steps"` | `"253 steps"` |
| `"one million rows in the database"` | `"1000000 rows in the database"` |
