...
const RULE_INFO = {
  ...
  punctuationNormalizer: {
    ...
    level: 'Level 1',
    levelClass: 'badge-blue',
    what: 'Compresses repeated punctuation marks and normalises ellipses. Collapses two or more consecutive exclamation marks to a single !, two or more consecutive question marks to a single ?, and four or more consecutive periods to the standard ellipsis ...',
    hasParams: false,
    exBefore: '"Is this correct?? I\'m sure!! Let me think...."',
    exAfter:  '"Is this correct? I\'m sure! Let me think..."',
    future: [
      'Support Chinese punctuation normalisation (！！！→ ！)',
      'Handle mixed punctuation sequences (!? or ?! → ?)',
      'Protect punctuation inside code blocks from being modified',
    ],
  },
  numberNormalizer: {
    ...
    level: 'Level 1',
    levelClass: 'badge-blue',
    what: 'Converts written English numbers and percentages to their Arabic numeral equivalents using a full place-value parser. Supports ones (zero–nineteen), tens (twenty–ninety), and scale words (hundred, thousand, million) in any combination. Percent phrases are processed first to avoid partial matches.',
    hasParams: false,
    exBefore: '"I need two hundred and fifty tokens — that is fifty percent of the budget."',
    exAfter:  '"I need 250 tokens — that is 50% of the budget."',
    future: [
...
function initUI() {
  ...
  document.querySelectorAll('.info-btn').forEach(btn => {
    const ruleId = btn.getAttribute('onclick').match(/'([^']+)'/)?.[1];
    if (ruleId) {
      btn.onclick = () => showModal(ruleId);
    }
  });
...
function renderPipelinePage(result) {
  ...
  steps.forEach((step, i) => {
    ...
    activeIdx++;
    const ruleId = RULE_ORDER.find(id => RULE_INFO[id] && RULE_INFO[id].name === step.ruleName) || '';
