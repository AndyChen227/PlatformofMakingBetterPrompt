...
/**
 * ============================================================
 *  MOCK Implementation — Structure Minimizer Rule
 * ============================================================
 * Current behaviour:
...
public class StructureMinimizerRule implements Rule {
    ...
    public StepResult apply(String inputText, RuleConfig config) {
        ...
        if (!stripped.equals(result)) {
            changes.add("[MOCK] 清理首尾空白");
            result = stripped;
        }

        if (changes.isEmpty()) {
            changes.add("[MOCK] Structure is already clean, no changes needed");
        }
